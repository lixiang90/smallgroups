...
/-! ## Descendants of `order16_wild_G2` (`C₈ ⋊ C₂`, semidihedral `SD₁₆`)

`order16_wild_G2 = C₈ ⋊[φ₂] C₂` where `φ₂` sends the generator `b` of `C₂` to the
automorphism `x ↦ x³` of `C₈` (`bab⁻¹ = a³`). As with `G12`, we record only the
trivial-cocycle branch (`a⁸ = 1`, `b² = 1`, and the conjugation relation holds exactly,
not just mod `z`), giving the split extension `G ≃* order16_wild_G2 × C₂`. -/

namespace Smallgroups.UsefulTheorems

/-- The trivial-cocycle branch over `order16_wild_G2`: both relator bits `a⁸, b²` vanish
and the conjugation defect vanishes, so `G` splits as `order16_wild_G2 × C₂`. -/
theorem order32_of_c8c2sd_quotient_trivial_case {G : Type*} [Group G] [Finite G]
    (hG : Nat.card G = 32) {z : G} (hzc : z ∈ Subgroup.center G) (hzp : orderOf z = 2)
    [hnorm : (Subgroup.zpowers z).Normal]
    (e : (G ⧸ Subgroup.zpowers z) ≃* order16_wild_G2)
    {ga gb : G}
    ...
    (hga8 : ga ^ 8 = 1) (hgb2 : gb ^ 2 = 1) (hrel : gb * ga * gb⁻¹ = ga ^ 3) :
    Nonempty (G ≃* order16_wild_G2 × Multiplicative (ZMod 2)) := by
  classical
  set φ := c2Action_phi2 with hφdef
  ...
  have hcomp : ∀ t : Multiplicative (ZMod 2),
    ...
    · have e1 : φ (Multiplicative.ofAdd (1 : ZMod 2)) (Multiplicative.ofAdd (1 : ZMod 8))
          = Multiplicative.ofAdd (3 : ZMod 8) := by rw [hφdef]; decide
      simp only [e1, hfn_val, hfg_val]
      have hv3 : (Multiplicative.toAdd (Multiplicative.ofAdd (3 : ZMod 8))).val = 3 := by decide
      ...
      simp only [pow_one]
      exact hrel.symm
  set s : order16_wild_G2 →* G := SemidirectProduct.lift fn fg hcomp with hsdef
  ...
  have hzrange : ∀ q : order16_wild_G2, s q = z → q = 1 := by
    intro q hqz
    have h1 := hπs q
    rw [hqz, hπz] at h1
    have h2 : q = e 1 := by
      ...
      simpa using this
    rw [h2, map_one]
  exact central_ext_split_iso_prod hG (card_order16_wild_G2) hzc hzp s hs_inj hzrange


/-! ## Twisted / doubled branches: `order16_wild_G2` descendants (continued)
...
through the existing `C₂`-action `c2Action_phi2`. -/
...
private noncomputable def phi4Act : Multiplicative (ZMod 4) →* MulAut C8g :=
  c2Action_phi2.comp redHom42

/-- The new order-32 group `C₈ ⋊[φ₄] C₄` (the `b`-doubled descendant of `G2`). -/
noncomputable abbrev order32_c8c4_u3 : Type :=
  SemidirectProduct C8g (Multiplicative (ZMod 4)) phi4Act

@[simp] theorem card_order32_c8c4_u3 : Nat.card order32_c8c4_u3 = 32 := by
  rw [SemidirectProduct.card]; simp

/-- The `b`-doubled branch over `order16_wild_G2`: `a⁸=1`, `b²=z`, `bab⁻¹=a³` (exact).
`G` is isomorphic to the new order-32 group `C₈ ⋊[φ₄] C₄`. -/
theorem order32_of_c8c2sd_quotient_doubled_b_case {G : Type*} [Group G] [Finite G]
    (hG : Nat.card G = 32) {z : G} (hzc : z ∈ Subgroup.center G) (hzp : orderOf z = 2)
    [hnorm : (Subgroup.zpowers z).Normal]
    (e : (G ⧸ Subgroup.zpowers z) ≃* order16_wild_G2)
    {ga gb : G}
    ...
    (hgb : (QuotientGroup.mk' (Subgroup.zpowers z)) gb
      = e.symm (SemidirectProduct.inr (Multiplicative.ofAdd (1 : ZMod 2))))
    (hga8 : ga ^ 8 = 1) (hgb2 : gb ^ 2 = z) (hrel : gb * ga * gb⁻¹ = ga ^ 3) :
    Nonempty (G ≃* order32_c8c4_u3) := by
  classical
  have hz2 : z ^ 2 = 1 := by rw [← hzp]; exact pow_orderOf_eq_one z
  have hzne1 : z ≠ 1 := by
    intro h; rw [h, orderOf_one] at hzp; exact absurd hzp (by norm_num)
  ...
  have hcomp : ∀ t : Multiplicative (ZMod 4),
    ...
    · have e1 : phi4Act (1 : Multiplicative (ZMod 4)) (Multiplicative.ofAdd (1 : ZMod 8))
        ...
        change c2Action_phi2 (redHom42 1) _ = _
    ...
    · have e1 : phi4Act (Multiplicative.ofAdd (1 : ZMod 4)) (Multiplicative.ofAdd (1 : ZMod 8))
          = Multiplicative.ofAdd (3 : ZMod 8) := by
        change c2Action_phi2 (redHom42 (Multiplicative.ofAdd (1 : ZMod 4))) _ = _
    ...
    · have e1 : phi4Act (Multiplicative.ofAdd (2 : ZMod 4)) (Multiplicative.ofAdd (1 : ZMod 8))
          = Multiplicative.ofAdd (1 : ZMod 8) := by
        change c2Action_phi2 (redHom42 (Multiplicative.ofAdd (2 : ZMod 4))) _ = _
    ...
    · have e1 : phi4Act (Multiplicative.ofAdd (3 : ZMod 4)) (Multiplicative.ofAdd (1 : ZMod 8))
          = Multiplicative.ofAdd (3 : ZMod 8) := by
        change c2Action_phi2 (redHom42 (Multiplicative.ofAdd (3 : ZMod 4))) _ = _
      ...
      rw [hstep]
      exact hrel.symm
  set s : order32_c8c4_u3 →* G := SemidirectProduct.lift fn fg hcomp with hsdef
  ...
  have hρcompat : ∀ g : Multiplicative (ZMod 4),
      (MonoidHom.id C8g).comp (phi4Act g).toMonoidHom
        = (c2Action_phi2 (redHom42 g)).toMonoidHom.comp (MonoidHom.id C8g) := by
    intro g; rfl
  set ρ : order32_c8c4_u3 →* order16_wild_G2 :=
  ...
  have hs_inj : Function.Injective s := by
    ...
    · exact hqleft
    · exact hqright1
  exact central_ext_full_iso hG card_order32_c8c4_u3 s hs_inj

/-! ## `z`-twisted conjugation branch: `order16_wild_G2` descendants (continued)
...
`a^(9-1) = z^(3+1) = 1`, which holds automatically since `9 ≡ 1 (mod 8)` (true for every
...
`b` acts on it via `(x,y) ↦ (3x, x+y)` (in additive `ZMod 8 × ZMod 2` coordinates) — this
...
"non-split" over `order16_wild_G2` itself, in the true cocycle sense). -/
...
/-- The involution `(x,y) ↦ (3x, x+y)` on `C₈ × C₂` (additively), giving the twisted
conjugation action of `b` on `⟨a,z⟩` for unit `3`. -/
private noncomputable def twistAut3 : C8C2g ≃* C8C2g where
  toFun p := (Multiplicative.ofAdd (3 * Multiplicative.toAdd p.1),
      Multiplicative.ofAdd
        ((ZMod.castHom (by norm_num : (2:ℕ) ∣ 8) (ZMod 2) (Multiplicative.toAdd p.1))
          + Multiplicative.toAdd p.2))
  invFun p := (Multiplicative.ofAdd (3 * Multiplicative.toAdd p.1),
      Multiplicative.ofAdd
        ((ZMod.castHom (by norm_num : (2:ℕ) ∣ 8) (ZMod 2) (Multiplicative.toAdd p.1))
          + Multiplicative.toAdd p.2))
  left_inv := by decide
...
@[simp] private theorem twistAut3_sq : twistAut3 ^ 2 = 1 := by
  apply MulEquiv.ext
  intro p
  change twistAut3 (twistAut3 p) = p
  revert p; decide

private lemma c2_two_cases_tw3 (a : Multiplicative (ZMod 2)) :
    a = 1 ∨ a = Multiplicative.ofAdd 1 := by
  have := show ∀ a : Multiplicative (ZMod 2), a = 1 ∨ a = Multiplicative.ofAdd 1 from by decide
  exact this a

/-- The `C₂`-action on `C₈ × C₂` given by `twistAut3`. Not `private`: `Distinctness.lean`
needs to name it to show `order32_c8c2tw_u3 ≃* order32_c8c2tw_u7`. -/
noncomputable def c8c2tw_phi3 : Multiplicative (ZMod 2) →* MulAut C8C2g where
  toFun g := if g = 1 then 1 else twistAut3
  map_one' := by simp
  map_mul' a b := by
    have htw2 : twistAut3 * twistAut3 = (1 : MulAut C8C2g) := by rw [← sq]; exact twistAut3_sq
    rcases c2_two_cases_tw3 a with (rfl|rfl) <;> rcases c2_two_cases_tw3 b with (rfl|rfl) <;>
...
noncomputable abbrev order32_c8c2tw_u3 : Type :=
  SemidirectProduct C8C2g (Multiplicative (ZMod 2)) c8c2tw_phi3

@[simp] theorem card_order32_c8c2tw_u3 : Nat.card order32_c8c2tw_u3 = 32 := by
  rw [SemidirectProduct.card]; simp

/-- The `z`-twisted branch over `order16_wild_G2`: `a⁸=1`, `b²=1`, `bab⁻¹=a³·z`.
`G` is isomorphic to the new order-32 group `(C₈ × C₂) ⋊[ψ₃] C₂`. -/
theorem order32_of_c8c2sd_quotient_twisted_case {G : Type*} [Group G] [Finite G]
    (hG : Nat.card G = 32) {z : G} (hzc : z ∈ Subgroup.center G) (hzp : orderOf z = 2)
    [hnorm : (Subgroup.zpowers z).Normal]
    (e : (G ⧸ Subgroup.zpowers z) ≃* order16_wild_G2)
    {ga gb : G}
    ...
    (hgb : (QuotientGroup.mk' (Subgroup.zpowers z)) gb
      = e.symm (SemidirectProduct.inr (Multiplicative.ofAdd (1 : ZMod 2))))
    (hga8 : ga ^ 8 = 1) (hgb2 : gb ^ 2 = 1) (hrel : gb * ga * gb⁻¹ = ga ^ 3 * z) :
    Nonempty (G ≃* order32_c8c2tw_u3) := by
  classical
  have hz2 : z ^ 2 = 1 := by rw [← hzp]; exact pow_orderOf_eq_one z
  have hzne1 : z ≠ 1 := by
    intro h; rw [h, orderOf_one] at hzp; exact absurd hzp (by norm_num)
  ...
  have hc2cases : ∀ x : Multiplicative (ZMod 2), x = 1 ∨ x = Multiplicative.ofAdd (1 : ZMod 2) :=
    by decide
  have hcomp : ∀ t : Multiplicative (ZMod 2),
      fn.comp (c8c2tw_phi3 t).toMonoidHom = (MulAut.conj (fg t)).toMonoidHom.comp fn := by
    intro t
    apply monoidHom_ext_prod_ofAdd_one
    · rcases hc2cases t with rfl | rfl
      · simp
      · have e1 : c8c2tw_phi3 (Multiplicative.ofAdd (1:ZMod 2))
            (Multiplicative.ofAdd (1:ZMod 8), (1 : Multiplicative (ZMod 2)))
            = (Multiplicative.ofAdd (3:ZMod 8), Multiplicative.ofAdd (1:ZMod 2)) := by decide
        ...
        have hv0 : (Multiplicative.toAdd (1:Multiplicative (ZMod 2))).val = 0 := by decide
        simp only [hfga_val, hfz_val, hfg_val, hv3, hv1a, hv1b, hv0, pow_zero, pow_one,
          mul_one]
        exact hrel.symm
    · rcases hc2cases t with rfl | rfl
      · simp
      · have e1 : c8c2tw_phi3 (Multiplicative.ofAdd (1:ZMod 2))
        ...
        exact hstep.symm
  set s : order32_c8c2tw_u3 →* G := SemidirectProduct.lift fn fg hcomp with hsdef
  ...
  have hρcompat : ∀ g : Multiplicative (ZMod 2),
      proj1.comp (c8c2tw_phi3 g).toMonoidHom
        = (c2Action_phi2 g).toMonoidHom.comp proj1 := by
    intro g
    apply monoidHom_ext_prod_ofAdd_one
    ...
    · simp only [MonoidHom.comp_apply, MulEquiv.coe_toMonoidHom]
      rcases hc2cases g with rfl | rfl <;> decide
  set ρ : order32_c8c2tw_u3 →* order16_wild_G2 :=
  ...
  exact central_ext_full_iso hG card_order32_c8c2tw_u3 s hs_inj
