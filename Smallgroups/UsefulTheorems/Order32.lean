...
In addition to the hand-developed branches below, `Order32Certificate` contains the
completed, GAP-indexed classification.  Its 51 central extensions are generated by
GF(2) row reduction and checked by Lean from finite multiplication tables.  Checked
automorphism-orbit paths prove completeness, explicit maps align every survivor with
`SmallGroup(32,j)`, and a kernel-computed local profile proves pairwise non-isomorphism.
...
`Order32/Distinctness.lean` proves `order32_partial_distinct_25`: 25 of the concrete
representatives constructed above are pairwise non-isomorphic. This improves the old
23-member result by separating one formerly tied pair with a powered-commuting-pair
count. That file remains a useful independent regression test for the original
hand-developed representatives.  The complete theorem is
`Order32Certificate.smallGroup32_isClassif`, with GAP index `j - 1` represented by
`smallGroup32` at that index.
-/
