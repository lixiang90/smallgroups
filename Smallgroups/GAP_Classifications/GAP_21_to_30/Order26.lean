...
/-!
# GAP-numbered classification of groups of order 26

`SmallGroup(26, 1) = DihedralGroup 13` and `SmallGroup(26, 2) = ℤ/26`.
The representatives are the imported pc groups `smallGroup 26 j`.
-/

namespace Smallgroups.GAP_Classifications.Order26

open Smallgroups.GAP Smallgroups.UsefulTheorems

/-- The GAP-numbered representatives of order `26`. -/
abbrev gapRep : Fin 2 → Type := rep2 (smallGroup 26 1) (smallGroup 26 2)
...
private noncomputable def gapEquivs :
    ∀ i, gapRep i ≃* rep2 (CyclicRep 26) (DihedralGroup 13) (swap i)
  | 0 => order26_1_equiv
  | 1 => order26_2_equiv

/-- **(1)+(2)+(3).** The GAP-numbered list `[SmallGroup(26, 1), SmallGroup(26, 2)]`
is complete and non-redundant. -/
theorem isClassif : IsClassif 26 gapRep :=
  IsClassif.of_equivs
    (IsClassif.of_perm Smallgroups.Classifications.Order26.isClassif swap) gapEquivs

/-- **(1) Exhaustiveness.** Every group of order `26` is isomorphic to a GAP
representative. -/
theorem classification {G : Type} [Group G] (h : Nat.card G = 26) :
...
end Smallgroups.GAP_Classifications.Order26
