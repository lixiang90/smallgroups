...
$N = 4 路 7, so there are exactly **four** groups of order $N up to isomorphism:
the cyclic group 鈩?28, 鈩?2 脳 鈩?14, 鈩?7 鈰奯{-1} 鈩?4, and 鈩?2 脳 D_14.
This is the p = 7 (p 鈮?3 mod 4) instance of the order-4p classification in
...
theorem classification (h : Nat.card G = 28) :
  ...
  simpa [RA, RB, RC, RD, show (4 : 鈩? * 7 = 28 by norm_num] using
