...
$N = 4 路 11, so there are exactly **four** groups of order $N up to isomorphism:
the cyclic group 鈩?44, 鈩?2 脳 鈩?22, 鈩?11 鈰奯{-1} 鈩?4, and 鈩?2 脳 D_22.
This is the p = 11 (p 鈮?3 mod 4) instance of the order-4p classification in
...
theorem classification (h : Nat.card G = 44) :
  ...
  simpa [RA, RB, RC, RD, show (4 : 鈩? * 11 = 44 by norm_num] using
...
theorem isClassif : IsClassif 44 (rep4 RA RB RC RD) := by
  simpa [RA, RB, RC, RD, show (4 : 鈩? * 11 = 44 by norm_num] using
